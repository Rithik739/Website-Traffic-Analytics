...
SELECT 
    dl.device_type,
    SUM(wtd.Session_Duration)/3600  as 'Total Session Duration(Hrs)',
    AVG(wtd.Session_Duration)/3600 as 'Average Session Duration (Hrs)',
    SUM(wtd.Page_Views_Per_Session) as 'Total Page Views'
FROM
    website_traffic_analytics.website_traffic_data wtd
        INNER JOIN
    device_lookup dl on dl.device_key = wtd.Device_Key
...
FROM
    website_traffic_analytics.website_traffic_data wtd
        INNER JOIN
    source_lookup sl on sl.source_key = wtd.Source_Key
...
With cte1 as (SELECT 
    dl.device_type,
COUNT(wtd.session_id) AS 'totsessions'
FROM
...
Select 
dl.device_type,
count(Session_Id) / max(cte1.totsessions) * 100 as Bounce_Rate
FROM
    website_traffic_analytics.website_traffic_data wtd 
    inner join device_lookup dl on 
		wtd.device_key = dl.device_key
        inner join cte1 on cte1.device_type= dl.device_type
    WHERE
...
FROM
    ...
    inner join device_lookup dl on 
		wtd.device_key = dl.device_key
        inner join cte1 on cte1.Device_Browser= dl.Device_Browser
    WHERE
		wtd.Page_Views_Per_Session < 2
		group by dl.Device_Browser;
...
FROM
    ...
    INNER JOIN
    source_lookup sl ON sl.Source_Key = wtd.Source_Key
        inner join cte1 on cte1.Source_Type= sl.source_type
        WHERE
		wtd.Page_Views_Per_Session < 2
		group by sl.Source_Type;
...
FROM
    ...
    inner join device_lookup dl on 
		wtd.device_key = dl.device_key
        inner join cte1 on cte1.Content_Segment= dl.Content_Segment
    WHERE
		wtd.Page_Views_Per_Session < 2
		group by dl.Content_Segment;
